...
#SBATCH --export=all
#SBATCH --output=Rlogs/IAS15/out_%A_%a.txt
#SBATCH --error=Rlogs/IAS15/err_%A_%a.txt

eval "$(/gscratch/uwb/ahuang54/miniconda3/bin/conda shell.bash hook)"
conda activate rebound


OUTDIR=results/run_IAS15RUN_${SLURM_ARRAY_TASK_ID}
mkdir -p $OUTDIR

python simulation_ias15.py \
